...
locations across Bentong, Pahang. Multilingual interface
...
This release migrates Bentong from the older forecast-only build onto
...
```
https://stanleywoosweeleong.github.io/weathernextforbentong/
```
...
On first launch the app seeds the eleven farms below. They are auto-favourited and
...
| Waterway Villa | 水道别墅 | 3.44917, 101.90056 | riverine | durian |
| Kg. Benus | 甘榜本努斯 | 3.50783, 101.92667 | sheltered basin | durian |
| Tras | 都赖 | 3.73622, 101.80792 | sheltered basin | durian |
| Kampung Hulu Lebu | 乌鲁乐布新村 | 3.57278, 101.92611 | sheltered basin | durian |
| Kampung Lebu | 甘榜乐布 | 3.61600, 101.93300 | sheltered basin | durian |
...
The app also seeds a default user display name (**文冬 / Bentong**),
which stays editable via **Edit Name** in the app.

This build carries the seed version **`bt-arch1`** (the lowland-architecture
...
```
wnext-weathernextforbentong-202606062158
```
...
- **Storage namespace:** `weathernextforbentong__*` keys in `localStorage`, isolated
  from other WeatherNext regional builds so data never collides.
- **Cloud sync:** Firebase, namespaced under `appId: wnext-ag-v41-weathernextforbentong`.
...
The icon shows farmers in a field under a blue sky. The boot screen background
(sky-blue `#8dd0ea`) is matched to the icon's sky. Note: Bentong sits in hilly
Pahang (~230–450 m); the build uses lowland-climate disease framing, which fits
this elevation band (well below the cool-highland threshold).
