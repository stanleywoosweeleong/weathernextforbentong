// ============================================================
// WeatherNext Service Worker — Bentong (文冬, Pahang)
// MIGRATED to the full Raub/Cameron microclimate architecture (was the older
// 1.0.156 forecast-only build, which had NO boot screen — one is added here).
...
// AI-greeting crop-owner fix, REAL model-run freshness header, AI prompt with
// per-farm coordinates (keeps Bentong name). Boot screen sky-blue (#8dd0ea) to
...
// (computeFog thresholds <=200m dense, <=1000m fog) instead of bandKey,
// so it matches the detail screen's fog index — a dawn broadcast no longer
// says 'dense fog' on a morning the detail shows clear; humidity alone can
// flag 'fog' only with no visibility reading, never 'dense'. (2) Dropped the
// misleading 'overnight/夜间/malam' wording from the low-dawn-count fog tag —
...
const CACHE_VERSION = 'wnext-weathernextforbentong-202606062158';
